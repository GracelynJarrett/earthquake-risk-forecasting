...
- **Prof-feedback follow-up experiments (presentation week):**
  - **Feature importance** — the model leans on `region` (base rate) + recent significant-activity magnitude/count; the added `avg_depth_30d` is barely used (coef ≈ +0.06), confirming the ablation.
  - **Precision/recall** — precision ≈ 0.30 at the 0.5 threshold (≈2 of 3 alarms false); per-region behavior is extreme (Japan flags everything — recall 1.000; Greece almost nothing — recall 0.093) → **per-region decision thresholds** are a Week-4/deployment need.
  - **"Remove-temporal" location-only test** — recent-activity features carry almost all the signal (performance drops near the floor without them); **fault-line is a real Greece-specific signal** (`faultline_only` Greece 0.350, beating the full model's 0.296).
  - **Region×feature interactions** — did **not** beat `base` (dilution + California cost); a linear model can't cleanly exploit the region structure → reinforces the XGBoost plan.
- **Week-4 backlog (prof + presentation feedback):** tune XGBoost with region×feature interactions; add per-region decision thresholds; test longer-window temporal features (avg magnitude 1/3/5-yr, all-time max) and explosion data; revisit the original data-leakage framing (no time-series model and as-of-day features, so shuffling the split may not actually leak).
